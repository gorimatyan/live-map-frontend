...
<Map token={token} initialRegion={initialRegion}>
      ...
      <Annotation latitude={46.207} longitude={6.1558} title="Jet d’eau" subtitle="Iconic landmark of Geneva" calloutElement={<CustomCalloutElement title="Jet d’eau" subtitle="Iconic landmark of Geneva" url="https://en.wikipedia.org/wiki/Jet_d%27Eau" />} calloutEnabled calloutOffsetX={-148} calloutOffsetY={-82}>
...
const CustomCalloutElement: React.FC<{ title: string; subtitle: string; url: string }> = ({ title, subtitle, url }) => {
    return (
      <div style={{ padding: '10px', backgroundColor: '#fff', borderRadius: '8px', boxShadow: '0 2px 6px rgba(0,0,0,0.3)' }}>
        <h3 style={{ margin: '0 0 5px 0' }}>{title}</h3>
        <p style={{ margin: '0 0 5px 0' }}>{subtitle}</p>
        <a href={url} target="_blank" rel="noopener noreferrer" style={{ color: '#007aff' }}>Website</a>
      </div>
    );
  };